...
import com.example.beproductive.databinding.FrgOnboarding3Binding
import com.example.beproductive.databinding.FrgOnboarding4Binding
import com.example.beproductive.databinding.FrgSplashBinding

class Onboarding4Fragment: Fragment() {
    var binding: FrgOnboarding4Binding? = null
    ...
    override fun onCreateView(
        ...
        binding = FrgOnboarding4Binding.inflate(inflater, container, false)
        return binding?.root
    }
    ...
    private fun listentAppEvent() {
        binding?.let { binding ->
            binding.btnNext.setOnClickListener {
                requireActivity().supportFragmentManager.beginTransaction().replace(R.id.ln_main, NoteFragment()).commit()
            }
        }
